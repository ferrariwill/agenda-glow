...
type CategoriaServico struct {
	...
	Icone             *string `db:"icone" json:"icone"`
}
...
func (s *CategoriaServicoService) Update(ctx context.Context, establishmentID, categoriaID, nome, icone string) error {
	nome = strings.TrimSpace(nome)
	if nome == "" {
		return fmt.Errorf("nome da categoria é obrigatório")
	}
...
`
	result, err := s.db.ExecContext(ctx, update, categoriaID, establishmentID, nome, icone)
	if err != nil {
		if pqErr, ok := err.(*pq.Error); ok && pqErr.Code == "23505" {
			return ErrCategoriaServicoNomeDuplicado
		}
...
func (s *CategoriaServicoService) BuscarPorID(ctx context.Context, establishmentID, categoriaID string) (*CategoriaServico, error) {
	const query = `
SELECT id, estabelecimento_id, nome, icone
FROM categorias_servicos
WHERE id = $1 AND estabelecimento_id = $2
`
	...
	if err := s.db.GetContext(ctx, &cat, query, categoriaID, establishmentID); err != nil {
