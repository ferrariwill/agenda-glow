...
// ServicoInsumoItem é um vínculo da ficha técnica (BOM) serviço ↔ insumo (visão sem link id).
...
// ServicoInsumo é o vínculo com id (CRUD item da ficha técnica).
...
func (s *ServicoInsumoService) Create(
	...
	if insumoID == "" || quantidadeUso <= 0 {
		return "", fmt.Errorf("insumo_id e quantidade_uso válida são obrigatórios")
	}
